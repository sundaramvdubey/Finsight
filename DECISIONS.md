...
| 2026-07-23 | Model: linear regression, volume vs. months-since-start (calendar-month index, not row index), temporal train/test split (last 4 real months held out) | Row-index time feature would silently compress the 5 excluded-month gaps; calendar-month index keeps spacing honest. Temporal (not random) split matches the projection use case — we're extrapolating forward, so testing on the most recent real months best approximates that | Final |
